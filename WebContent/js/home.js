define(['jquery_SuperSlide','common','handlebar'],function(jquery_SuperSlide,common,Handlebars){
	...
	function getFloors(){
		$.ajax({
			//首页楼层商品数据接口
			url:baseUrl+"product/findfloor.do",
			xhrFields:{withCredentials:true},
			crossDomain:true,
			success:function(rs){
				//判断是否成功
				...
				for(var i=0;i<data3.length;i++){
					data2[i].iconUrl=baseUrl+data3[i].iconUrl;
				}
				$("#floor_three_m").html();
				...
				for(var i=0;i<data4.length;i++){
					data2[i].iconUrl=baseUrl+data4[i].iconUrl;
				}
				$("#floor_four_m").html();
